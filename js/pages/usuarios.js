/**
 * ETTUR - Gestión de Usuarios v3.0
...
const PageUsuarios = {
                ...
                <div class="card-ettur fade-in mb-3">
                    ...
                    <div class="card-body-inner">
                        ${users.map(u => {
                            ...
                            const montoP = u.tipo_trabajador === 'personalizado' && u.monto_personalizado ? ` · S/${parseFloat(u.monto_personalizado).toFixed(2)} ${u.frecuencia_personalizado || 'sem.'}` : '';
                            ...
                            <div class="user-item">
                                ...
                                <div class="user-actions">
                                    ...
                                    <button class="btn btn-outline-${u.activo ? 'warning' : 'success'} btn-sm" onclick="PageUsuarios.toggleEstado(${u.id}, '${escapedName}', ${u.activo})" title="${u.activo ? 'Baja' : 'Activar'}"><i class="bi bi-${u.activo ? 'pause' : 'play'}"></i></button>
                                    ${u.rol !== 'admin' ? `<button class="btn btn-outline-danger btn-sm" onclick="PageUsuarios.eliminar(${u.id}, '${escapedName}')" title="Eliminar"><i class="bi bi-trash"></i></button>` : ''}
                                </div>
                            </div>`;
                        }).join('')}
                    </div>
    ...
    async showForm(user) {
        ...
        // Obtener fecha lanzamiento si existe
            ...
            <form id="form-usuario">
                ...
                <div id="trabajador-fields" style="${(!user || user.rol_id == 3) ? '' : 'display:none'}">
                    ...
                    <div id="tipo-detalle" class="alert alert-light border mb-3" style="font-size:0.8rem">
                        <div id="detalle-normal" ${user?.tipo_trabajador && user.tipo_trabajador !== 'normal' ? 'style="display:none"' : ''}><i class="bi bi-person-fill text-primary"></i> <strong>Normal</strong> — Semanal — ${textoNormal}</div>
                        <div id="detalle-especial" style="${user?.tipo_trabajador == 'especial' ? '' : 'display:none'}"><i class="bi bi-star-fill text-info"></i> <strong>Especial</strong> — Semanal — ${textoEspecial}</div>
                        <div id="detalle-mensual" style="${user?.tipo_trabajador == 'mensual' ? '' : 'display:none'}"><i class="bi bi-calendar-month-fill text-success"></i> <strong>Mensual</strong> — ${textoMensual}</div>
                        <div id="detalle-personalizado" style="${user?.tipo_trabajador == 'personalizado' ? '' : 'display:none'}"><i class="bi bi-gear-fill text-warning"></i> <strong>Personalizado</strong> — Monto y frecuencia manual</div>
                    </div>
                    ...
                    <div class="alert alert-info border mb-3" style="font-size:0.8rem">
                        <i class="bi bi-info-circle-fill"></i> <strong>Configuración de Fechas</strong><br>
                        <small>La <strong>fecha de deuda</strong> es desde cuándo el trabajador debe. La <strong>fecha de lanzamiento</strong> es desde cuándo la app empieza a cobrar normalmente. La diferencia genera la deuda histórica.</small>
                    </div>
                    <div class="row g-2 mb-3">
                        <div class="col-6">
                            <label class="form-label" style="font-size:0.8rem">📅 Fecha Inicio Deuda *</label>
                            <input type="date" class="form-control" id="usr-fecha-inicio" value="${user?.fecha_inicio_cobro || ''}">
                            <small class="text-muted">Desde cuándo debe</small>
                        </div>
                        <div class="col-6">
                            <label class="form-label" style="font-size:0.8rem">🚀 Fecha Lanzamiento App</label>
                            <input type="date" class="form-control" id="usr-fecha-lanzamiento" value="${fechaLanzamiento}">
                            <small class="text-muted">Desde cuándo corre la app</small>
                        </div>
                    </div>
                </div>
        ...
        const footer = `
            ...
            ${isEdit ? `<button class="btn btn-warning btn-sm" onclick="PageUsuarios.resetPlaca(${user.id}, '${user.placa || ''}')"><i class="bi bi-truck-front"></i> Placa</button>` : ''}
        ...
        document.getElementById('btn-save-user').onclick = async () => {
            ...
            if (data.rol_id == 3) {
                ...
                if (!data.fecha_inicio_cobro) {
                    document.getElementById('usr-error').textContent = 'La fecha de inicio de deuda es obligatoria';
                    document.getElementById('usr-error').classList.remove('d-none'); return;
                }
    ...
    toggleEstado(id, nombre, activo) {
        UI.confirm(activo ? 'Dar de Baja' : 'Activar', `¿Desea ${activo ? 'dar de baja' : 'activar'} a <strong>${nombre}</strong>?`,
            ...
            activo ? 'Dar de Baja' : 'Activar', activo ? 'warning' : 'success');
    },
    eliminar(id, nombre) {
        UI.confirm('Eliminar Usuario', `¿Eliminar permanentemente a <strong>${nombre}</strong>?<br><small class="text-danger">Se borrarán todos sus pagos.</small>`,

/**
 * ETTUR - Gestión de Usuarios v3.2
 * Buscador, deudas visibles, filtro por tipo
 */
const PageUsuarios = {
    tarifasCache: null,
    deudasCache: null,
    ...
    async cargarDeudas() {
        if (this.deudasCache) return this.deudasCache;
        try {
            const res = await API.getDeudas();
            if (res.success) {
                this.deudasCache = {};
                res.data.trabajadores.forEach(t => { this.deudasCache[t.id] = t; });
            }
        } catch(e) {}
    ...
    async render() {
        ...
        const [resUsuarios] = await Promise.all([API.getUsuarios()]);
        if (!resUsuarios.success) { main.innerHTML = `<div class="alert alert-danger">${resUsuarios.message}</div>`; return; }

        // Cargar deudas en background
        this.deudasCache = null;
        this.cargarDeudas().then(() => this.renderList(resUsuarios.data));

        this.allUsuarios = resUsuarios.data;
        this.renderShell(main);
        this.renderList(resUsuarios.data);
    },
                    ...
                    <div class="row g-2 align-items-center">
                        <div class="col">
                            <div class="input-group input-group-sm">
                                ...
                                <input type="text" class="form-control" id="usr-search" placeholder="Buscar por nombre, DNI o placa..." oninput="PageUsuarios.filtrar()">
                            </div>
                        </div>
                        <div class="col-auto">
                            <select class="form-select form-select-sm" id="usr-filtro-tipo" onchange="PageUsuarios.filtrar()" style="width:auto">
    ...
    filtrar() {
        this.filtroBusqueda = (document.getElementById('usr-search')?.value || '').toLowerCase();
        this.filtroTipo = document.getElementById('usr-filtro-tipo')?.value || '';
        this.renderList(this.allUsuarios);
    },
    ...
    renderList(usuarios) {
        ...
        // Aplicar filtro tipo
        if (this.filtroTipo) {
            filtered = filtered.filter(u => u.tipo_trabajador === this.filtroTipo);
        }

        const grouped = { admin: [], coadmin: [], trabajador: [] };
        filtered.forEach(u => { if (grouped[u.rol]) grouped[u.rol].push(u); });

        const deudas = this.deudasCache || {};
                        ...
                        ${users.map(u => {
                            ...
                            if (deuda && deuda.deuda_total > 0) {
                                deudaHtml = `
                                    <div class="d-flex gap-2 mt-1" style="font-size:0.7rem">
                                        <span class="text-danger fw-bold"><i class="bi bi-exclamation-circle"></i> Deuda: ${CONFIG.formatMoney(deuda.deuda_total)}</span>
                                        ${deuda.deuda_corriente > 0 ? `<span class="text-primary">Corr: ${CONFIG.formatMoney(deuda.deuda_corriente)}</span>` : ''}
                                        ${deuda.deuda_historica > 0 ? `<span class="text-secondary">Hist: ${CONFIG.formatMoney(deuda.deuda_historica)}</span>` : ''}
                                        ${deuda.pagos_por_validar > 0 ? `<span class="badge bg-warning text-dark">${deuda.pagos_por_validar} pendiente(s)</span>` : ''}
                                    </div>`;
                            } else if (deuda && deuda.deuda_total <= 0) {
                                deudaHtml = `<div style="font-size:0.7rem" class="text-success mt-1"><i class="bi bi-check-circle-fill"></i> Al día</div>`;
                            }
                            ...
                            <div class="user-item">
                                ...
                                <div class="user-info-text">
                                    ...
                                    <div class="user-detail">DNI: ${u.dni} · 🚗 ${u.placa || '—'} ${tipoLabel}</div>
                                ...
                                <div class="user-actions">
                                    ...
                                    ${u.rol === 'trabajador' ? `<button class="btn btn-outline-info btn-sm" onclick="PageUsuarios.editarDocumentos(${u.id}, '${escapedName}')" title="Docs"><i class="bi bi-folder2-open"></i></button>` : ''}
        ...
        let html = '';
        if (!this.filtroTipo) {
            html += renderGroup('Administradores', 'shield-fill', grouped.admin, 'admin');
            html += renderGroup('Coadministradores', 'person-badge', grouped.coadmin, 'coadmin');
        }
        html += renderGroup('Trabajadores', 'people', grouped.trabajador, 'trabajador');

        if (filtered.length === 0) {
            html = `<div class="text-center text-muted py-4"><i class="bi bi-search" style="font-size:2rem;opacity:0.3"></i><p class="mt-2">No se encontraron resultados</p></div>`;
        }
    ...
    async showForm(user) {
        ...
        if (isEdit && user.rol_id == 3) {
            try { const resP = await API.getPeriodosPendientes(user.id); if (resP.success) { fechaLanzamiento = resP.data.fecha_lanzamiento || ''; } } catch(e) {}
        }
        const body = `
        ...
        document.getElementById('btn-save-user').onclick = async () => {
            const data = { nombres: document.getElementById('usr-nombres').value.trim(), apellidos: document.getElementById('usr-apellidos').value.trim(), dni: document.getElementById('usr-dni').value.trim(), placa: document.getElementById('usr-placa').value.trim().toUpperCase(), telefono: document.getElementById('usr-telefono').value.trim(), rol_id: parseInt(document.getElementById('usr-rol').value) };
            if (!data.nombres || !data.apellidos || !data.dni || !data.placa) { document.getElementById('usr-error').textContent = 'Complete campos obligatorios'; document.getElementById('usr-error').classList.remove('d-none'); return; }
            if (data.rol_id == 3) {
                ...
                if (data.tipo_trabajador === 'personalizado') { data.monto_personalizado = parseFloat(document.getElementById('usr-monto-pers').value); data.frecuencia_personalizado = document.getElementById('usr-freq-pers').value; if (!data.monto_personalizado) { document.getElementById('usr-error').textContent = 'Monto requerido'; document.getElementById('usr-error').classList.remove('d-none'); return; } }
            }
            let res;
            if (isEdit) { data.id = user.id; res = await API.editarUsuario(data); } else { res = await API.crearUsuario(data); }
            if (res.success) { modal.hide(); UI.toast(res.message, 'success'); this.deudasCache = null; this.render(); }
    ...
    async editarDocumentos(userId, nombre) {
        ...
        const docs = res.data;
        const tipos = [
            { key: 'licencia', icon: 'card-heading', label: 'Licencia de Conducir', hasCategoria: true, hasNumero: true },
            { key: 'soat', icon: 'shield-check', label: 'SOAT' },
            { key: 'revision_tecnica', icon: 'wrench-adjustable', label: 'Revisión Técnica' },
            { key: 'tarjeta_circulacion', icon: 'credit-card-2-front', label: 'T. Circulación' },
            { key: 'tarjeta_operatividad', icon: 'file-earmark-check', label: 'T. Operatividad' }
        ];
        const tabs = tipos.map((t, i) => `<button class="btn btn-sm ${i === 0 ? 'btn-primary' : 'btn-outline-secondary'} doc-tab-btn" data-tab="doc-tab-${t.key}" onclick="PageUsuarios.switchDocTab('${t.key}')" style="font-size:0.65rem;padding:0.2rem 0.4rem"><i class="bi bi-${t.icon}"></i> ${t.label.split(' ')[0]}</button>`).join('');
        const panels = tipos.map((t, i) => {
            const doc = docs[t.key];
            return `
            <div id="doc-tab-${t.key}" class="doc-panel" style="${i > 0 ? 'display:none' : ''}">
                <h6><i class="bi bi-${t.icon}"></i> ${t.label}</h6>
                ${t.hasNumero ? `<div class="mb-2"><label class="form-label" style="font-size:0.75rem">Número</label><input type="text" class="form-control form-control-sm" id="doc-${t.key}-numero" value="${doc?.numero || ''}" placeholder="M + DNI"></div>` : ''}
                ${t.hasCategoria ? `<div class="mb-2"><label class="form-label" style="font-size:0.75rem">Categoría</label><select class="form-select form-select-sm" id="doc-${t.key}-categoria"><option value="">Seleccionar</option>${['A1','A2A','A2B','A3A','A3B','A3C'].map(c => `<option value="${c}" ${doc?.categoria === c ? 'selected' : ''}>${c}</option>`).join('')}</select></div>` : ''}
                <div class="row g-2 mb-2">
                    <div class="col-6"><label class="form-label" style="font-size:0.75rem">Inicio</label><input type="date" class="form-control form-control-sm" id="doc-${t.key}-inicio" value="${doc?.fecha_inicio || ''}"></div>
                    <div class="col-6"><label class="form-label" style="font-size:0.75rem">Vencimiento</label><input type="date" class="form-control form-control-sm" id="doc-${t.key}-vencimiento" value="${doc?.fecha_vencimiento || ''}"></div>
                </div>
                ${doc && doc.dias_restantes !== null ? `<div class="mb-2" style="font-size:0.75rem">${doc.vencido ? '<span class="badge bg-danger">VENCIDO</span> Hace ' + Math.abs(doc.dias_restantes) + ' días' : doc.por_vencer ? '<span class="badge bg-warning text-dark">POR VENCER</span> ' + doc.dias_restantes + ' días' : '<span class="badge bg-success">VIGENTE</span> ' + doc.dias_restantes + ' días'}</div>` : ''}
                <button class="btn btn-primary-ettur btn-ettur btn-sm w-100" onclick="PageUsuarios.guardarDocumento(${userId}, '${t.key}')"><i class="bi bi-save"></i> Guardar</button>
            </div>`;
        }).join('');
        UI.modal(`Documentos — ${nombre}`, `<div class="d-flex gap-1 flex-wrap mb-3">${tabs}</div><div>${panels}</div>`, '<button class="btn btn-outline-secondary" data-bs-dismiss="modal">Cerrar</button>');
    },
    switchDocTab(key) {
        ...
        const panel = document.getElementById('doc-tab-' + key); if (panel) panel.style.display = '';
        const btn = document.querySelector(`[data-tab="doc-tab-${key}"]`); if (btn) btn.className = btn.className.replace('btn-outline-secondary', 'btn-primary');
    },
    async guardarDocumento(userId, tipo) {
        const data = { usuario_id: userId, tipo_documento: tipo, fecha_inicio: document.getElementById(`doc-${tipo}-inicio`)?.value || '', fecha_vencimiento: document.getElementById(`doc-${tipo}-vencimiento`)?.value || '' };
        if (tipo === 'licencia') { data.numero = document.getElementById(`doc-${tipo}-numero`)?.value || ''; data.categoria = document.getElementById(`doc-${tipo}-categoria`)?.value || ''; }
        const res = await API.guardarDocumento(data);
        if (res.success) { UI.toast('Guardado', 'success'); UI.closeModal(); setTimeout(() => { const nombre = ''; this.editarDocumentos(userId, nombre); }, 300); }
        else { UI.toast(res.message, 'error'); }
    },
    toggleTipoTrab() { document.getElementById('trabajador-fields').style.display = document.getElementById('usr-rol').value == '3' ? '' : 'none'; },
    togglePersonalizado() { document.getElementById('personalizado-fields').style.display = document.getElementById('usr-tipo-trab').value === 'personalizado' ? '' : 'none'; },
    toggleEstado(id, nombre, activo) {
        UI.confirm(activo ? 'Dar de Baja' : 'Activar', `¿${activo ? 'Dar de baja' : 'Activar'} a <strong>${nombre}</strong>?`,
            async () => { const res = await API.toggleUsuario(id); if (res.success) { UI.toast(res.message, 'success'); this.deudasCache = null; this.render(); } else { UI.toast(res.message, 'error'); } },
    ...
    eliminar(id, nombre) {
        UI.confirm('Eliminar', `¿Eliminar a <strong>${nombre}</strong>?<br><small class="text-danger">Se borrarán todos sus datos.</small>`,
            async () => { const res = await API.eliminarUsuario(id); if (res.success) { UI.toast(res.message, 'success'); this.deudasCache = null; this.render(); } else { UI.toast(res.message, 'error'); } },
            'Eliminar', 'danger');
    },
    resetPlaca(id, placaActual) {
        ...
        const modal = UI.modal('Cambiar Placa', body, `<button class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancelar</button><button class="btn btn-warning" id="btn-reset-placa">Actualizar</button>`);
